            ...
            <div className="bg-white p-8 rounded-lg w-11/12 max-w-3xl shadow-2xl">
                ...
                <div className="mb-6 border-b pb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <p><span className="font-semibold">Name:</span> {order.customerDetails?.name || order.deliveryDetails?.name || "N/A"}</p>
                        <p><span className="font-semibold">Email:</span> {order.customerDetails?.email || order.deliveryDetails?.email || "N/A"}</p>
                        <p><span className="font-semibold">Phone:</span> {order.customerDetails?.phone || order.deliveryDetails?.phone || "N/A"}</p>
                        <p>
                            <span className="font-semibold">Address:</span>{" "}
                            {order.customerDetails?.flatno || order.deliveryDetails?.flatno || "N/A"}, 
                            {order.customerDetails?.roadNo || order.deliveryDetails?.roadNo || "N/A"}
                        </p>
                    </div>
                </div>
                                ...
                                {(order.items || []).length > 0 ? (
                                    order.items.map((item, index) => (
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="py-2 px-4 border">{convertToLKR?.(item.price * item.quantity) || "0.00"}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <p className="text-2xl font-bold text-red-600">LKR {convertToLKR?.(order.totalPrice) || "0.00"}</p>
                </div>
